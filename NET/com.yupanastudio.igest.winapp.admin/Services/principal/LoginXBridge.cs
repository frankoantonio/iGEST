...
namespace com.yupanastudio.igest.winapp
{
    public class LoginXBridge
    {
        public async Task<LoginX> validarUsuario(int idEmpresa, string usu, string pwd)
        {
            ...
            Vars.httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(byteArray));
            Vars.httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));

            HttpResponseMessage response = await Vars.httpClient.GetAsync(Vars.urlServerApp + $"/admin/autenticar/{idEmpresa}/{usu}");
