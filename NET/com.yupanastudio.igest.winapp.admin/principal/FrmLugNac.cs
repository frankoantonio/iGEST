...
namespace com.yupanastudio.igest.winapp
{
    public partial class FrmLugNac : FrmMaster
    {
        ...
        private void FrmLugNac_Load(object sender, EventArgs e)
        {
            ...
            {
                Distrito obDist = Vars.lsDistritos.Where(x => x.idDistrito == obLgNc.idDist).FirstOrDefault();
        ...
        private void cmbPais_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmbPais.SelectedItem != null)
                cmbDpto.DataSource = Vars.lsDepartamentos.Where(x => x.idPais == ((Pais)cmbPais.SelectedItem).idPais).ToList();
        }

        private void cmbDep_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbDpto.SelectedItem != null)
                cmbProv.DataSource = Vars.lsProvincias.Where(x => x.idDepartamento == ((Departamento)cmbDpto.SelectedItem).idDepartamento).ToList();
        }

        private void cmbProv_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbProv.SelectedItem != null)
                cmbDist.DataSource = Vars.lsDistritos.Where(x => x.idProvincia == ((Provincia)cmbProv.SelectedItem).idProvincia).ToList();
        }
