...
{
    public partial class UcEstudianteCole : UcMaster
    {
        ...
        private void btnDireccion_Click(object sender, EventArgs e)
        {
            ...
            FrmDireccionFind frm = new FrmDireccionFind(obDireccion, obEstCol, true, this.obOpcion.soloLectura);
            frm.ShowDialog();
            if (frm.operOK)
            {
        ...
        private void btnNewInsc_Click(object sender, EventArgs e)
        {
            ...
            if (lsFams.Where(x => x.apoderado).Count() != 1)
            {
        ...
        {
            lblSubtitle.Text = "Registro de estudiantes e inscripciones";
            mostrarEstudiante();
